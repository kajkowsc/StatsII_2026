...
results_binom <- optim(fn=binom_likelihood, outcome=y_binom, input=X, par=0:1, hessian=T, method="BFGS") #Hessian = True is the standard errors of the curvature 
...
y_norm <- X*real_beta + rnorm(100, 0, 0.5) #rnorm is adding noise so the y values dont perfectly fit the line
